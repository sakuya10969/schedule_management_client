...
import { formatCandidate, filterCandidates } from "@/features/appointment/utils";
...
export const useScheduleForm = ({ apiUrl }: UseScheduleFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const formattedCandidate = 
      selectedCandidate === "none" ? "なし" : formatCandidate(selectedCandidate.split(", "));

    if (!window.confirm(`以下の日程で登録を行います:\n${formattedCandidate}\n\nこちらの内容で間違いないですか？`)) {
      return;
    }
  ...
  const filteredCandidates = filterCandidates(candidates, minTime, maxTime, selectedDays);
